...
export class AgendaService {
    ...
    getAgendas(): Observable<AgendaView[]> {
        ...
        return data.map((response: Response) => <AgendaView[]>response.json())
                    // .do(data => console.log('getAgendas: ' +  JSON.stringify(data)))
                    .catch(this.handleError);
    }

    getAgendasRealidas(isFavorito: boolean): Observable<AgendaView[]> {
        if (isFavorito) {
            return this.getAgendas()
                        .map((lista: AgendaView[]) => lista.filter((data: AgendaView) => (new Date(data.dataFim) < new Date()) && data.favorito ))
                        .catch(this.handleError);
        }
        return this.getAgendas()
